...
namespace ce {
	...
	std::string ceSerial::ReadChar(bool& success, int length)
	{

		success = false;
		if (!IsOpened()) { return "none"; }
		DWORD dwRead;
		DWORD Dlength = 1;
		char x = '?';
		...
		if (!fWaitingOnRead) {
			...
			if (!ReadFile(hComm, data, Dlength, &dwRead, &osReader)) {
		...
		if (fWaitingOnRead) {
			...
			case WAIT_OBJECT_0:
				if (!GetOverlappedResult(hComm, &osReader, &dwRead, FALSE)) {/*Error*/ }
					...
					if (dwRead == Dlength) success = true;
					fWaitingOnRead = FALSE;
